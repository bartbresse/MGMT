<div class="wrap">
	<div class="content container">
		<div class="row">
		    <div class="col-md-12">
	<h2 class="page-title"><?=$lang->translate('New language');?></h2>
		    </div>
		</div>
		<div class="row">
		    <div class="col-md-7">
		        <section class="widget">
					...
					<div class="body">
						<div id="user-form" class="form-horizontal label-left" data-parsley-priority-enabled="false" novalidate="">
							<input type="hidden" id="id" class="form-control8975" value="<? if(isset($_GET['id'])){ echo $_GET['id']; }else{ echo $entityid; } ?>" />
							<fieldset>
						...
						<div class="control-group" id="engroup">
							...
							<div class="controls form-group">
								<div><?php echo $form->render("en"); ?></div>
							</div>
						</div>
						...
						<div class="control-group" id="frgroup">
							...
							<div class="controls form-group">
								<div><?php echo $form->render("fr"); ?></div>
							</div>
						</div>
						...
						<div class="control-group" id="degroup">
							...
							<div class="form-actions" class="pull-right">
								<button type="submit" onclick="save({filearray:filearray, class:'form-control8975',action:'<?=$this->url->get('mgmtlanguage/add');?>',goto:'<?=$this->url->get('mgmtlanguage/index');?>'});" class="btn btn-primary">Opslaan</button>
								<button type="submit" onclick="save({filearray:filearray, class:'form-control8975',action:'<?=$this->url->get('mgmtlanguage/add');?>',goto:'<?=$this->url->get('mgmtlanguage/new');?>'});" class="btn btn-primary">Opslaan + Nieuw</button>
								<button type="button" onclick="go('<?=$this->url->get('mgmtlanguage/index');?>');" class="btn btn-default">Cancel</button>
							</div>
						</div>
					</div>
				</section>
