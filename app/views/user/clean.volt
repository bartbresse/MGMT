{{ content() }}
	...
	<header>
		<? if(count($users->items) > 39){ $this->partial("table/numberofresults"); } ?>		  	
	</header>
	<div>
   <? if(isset($users->items) && count($users->items) > 0){ ?>
	...
	<div class="clearfix">		
		<div class="pull-right">
			...
			<button class="btn btn-primary" onclick="go('<?php echo $this->url->get('user/new'.$id);?>');">
