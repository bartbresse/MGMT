...
<div class="wrap">
	 <div class="content container">
		    <div class="row">
		        <div class="col-md-12">
		            <h2 class="page-title">Evenementen<small></small></h2>
		        </div>
		    </div>
		    <div class="row">
		        <div class="col-md-12">
		            <section class="widget" id="mgmt-table-container">
		                ...
		                <div class="body">
		               <? if(isset($events->items) && count($events->items) > 0){ ?>
								...
								<?
									if(count($events) > 0){
										...
										?><tr id="<?=$event->id;?>"><?	
											foreach($columns as $column)
											{
												 if(in_array($column,$columns)){ ?><td class="<?=$column;?>"><?=$event->$column;?></td><? }
											}
											?>
											<td>
											...
											<td id="bewerken" style="width:100px;">
												...
												{ ?><a onclick="del('<?=$event->id;?>','event');"><i class="fa fa-trash-o"></i></a><?	} 
												*/ ?>
												<a href="<?php echo  $this->url->get("event/view&id=".$event->id);?>"><i class="fa fa-search"></i></a>
												<a href="<?php echo  $this->url->get('event/edit&id='.$event->id);?>"><i class="fa fa-edit"></i></a>
												<a style="cursor:pointer;" onclick="del('<?=$event->id;?>','event');"><i class="fa fa-trash-o"></i></a>
										    </td>
										 </tr><?		
										}
									}
