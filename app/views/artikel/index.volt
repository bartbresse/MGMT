		    ...
		    <div class="row">
		        <div class="col-md-12">
		            <section class="widget" id="mgmt-table-container">
		                ...
		                <div class="body">
		               <? if(isset($artikels->items) && count($artikels->items) > 0){ ?>
		                    <table id="datatable-table" class="table table-striped">
		                        <thead class="table table-striped table-editable no-margin">
		            			   <tr>
									...
									<?
									$cc = 0;
									foreach($artikels->items[0] as $key => $value)
									{
		                        ...
		                        <tbody class="table table-striped table-editable no-margin">
									...
									if(count($artikels) > 0){
										...
										{
										?><tr id="<?=$artikel->id;?>">
											<td id="select"><input type="checkbox" class="selected" /></td>
										<?	foreach($artikel as $key => $value)
											...
											<td id="bewerken" style="width:100px;">
												...
												{ ?><a onclick="del('<?=$artikel->id;?>','artikel');"><i class="fa fa-trash-o"></i></a><?	} 
												*/ ?>
												<a href="<?php echo  $this->url->get("artikel/view&id=".$artikel->id);?>"><i class="fa fa-search"></i></a>
												<a href="<?php echo  $this->url->get('artikel/edit&id='.$artikel->id);?>"><i class="fa fa-edit"></i></a>
												<a onclick="del('<?=$artikel->id;?>','artikel');"><i class="fa fa-trash-o"></i></a>
						...
						<div class="clearfix">		
							...
							<div>
								<select id="export-options">
									<option value="delete"><?=$lang->translate('Verwijder geselecteerd');?></option>
									<option value="csv"><?=$lang->translate('Exporteren naar Excel');?></option>
								</select>
