{{ content() }}
	<header>
		<? if(count($indexartikels->items) > 39){ ?>
	...
	<div>
   <? if(isset($indexartikels->items) && count($indexartikels->items) > 0){ ?>
		<table id="datatable-table" class="table table-striped">
			<thead class="table table-striped table-editable no-margin">
			   <tr>
				<?
				$cc = 0;
				foreach($indexartikels->items[0] as $key => $value)
				{
					//check if column is visible
					if(in_array($key,$artikelcolumns))
					{ 
			...
			<?	if(count($indexartikels) > 0){
					foreach($indexartikels->items as $artikel)
					{
					?><tr id="<?=$artikel->id;?>"><?	
						foreach($artikel as $key => $value)
						{
							if(in_array($key,$artikelcolumns)){ ?><td class="<?=$key;?>"><?=$value?></td><? }
						}
						?>
						<td id="bewerken" style="width:100px;">
						<?	if(in_array('edit',$actions))
							{
								?><a href="<?php echo  $this->url->get("artikel/view&id=".$artikel->id);?>"><i class="fa fa-search"></i></a><?
							}
							if(in_array('edit',$actions))
							{
								?><a href="<?php echo  $this->url->get('artikel/edit&id='.$artikel->id);?>"><i class="fa fa-edit"></i></a><?
							}
							if(in_array('delete',$actions))
							{
								?><a onclick="del('<?=$artikel->id;?>','<? if(isset($artikel->titel)){ echo $artikel->titel; } ?>');"><i class="fa fa-trash-o"></i></a><?
							} ?>
					   </td>
					 </tr><?		
					}
	...
	<div class="clearfix">		
		<div class="pull-right">
			...
			<button class="btn btn-primary" onclick="go('<?php echo $this->url->get('artikel/new'.$id);?>');">
