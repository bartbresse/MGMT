...
<div class="wrap">
	<div class="content container">
		<div class="row">
		    <div class="col-md-12">
	<h2 class="page-title"><? if(isset($_GET['id'])){ echo 'Wijzig'; }else{echo 'Nieuw';} ?> <?=$lang->translate('doelgroep');?> <!--<small>Out of the box form</small>--></h2>
		    </div>
		</div>
		<div class="row">
		    <div class="col-md-7">
					...
					<div class="body">
							...
							<div class="control-group" id="beschrijvinggroup">
								...
								<div class="controls form-group" style="border:1px solid #ccc;">
									<?	//crop images settings
										...
										if(isset($doelgroep->beschrijving)){ $config['content'] = $doelgroep->beschrijving; }
										?>
						...
						<script>$('#lastedit').datetimepicker();</script>
							...
							<div class="form-actions" class="pull-right">
								<button type="submit" onclick="save({filearray:filearray, class:'form-control',action:'<?=$this->url->get('doelgroep/add');?>',goto:'<?=$this->url->get('doelgroep/index');?>'});" class="btn btn-primary">Opslaan</button>
								<button type="submit" onclick="save({filearray:filearray, class:'form-control',action:'<?=$this->url->get('doelgroep/add');?>',goto:'<?=$this->url->get('doelgroep/new');?>'});" class="btn btn-primary">Opslaan + Nieuw</button>
								<button type="button" onclick="go('<?=$this->url->get('doelgroep/index');?>');" class="btn btn-default">Cancel</button>
							</div>
						</div>
					</div>
				</section>
