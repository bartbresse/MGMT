{{ content() }}
	<header>
		<? if(count($indexcontacts->items) > 39){ ?>
	...
	<div>
   <? if(isset($indexcontacts->items) && count($indexcontacts->items) > 0){ ?>
		<table id="datatable-table" class="table table-striped">
			<thead class="table table-striped table-editable no-margin">
			   <tr>
				<?
				$cc = 0;
				foreach($indexcontacts->items[0] as $key => $value)
				{
					//check if column is visible
					if(in_array($key,$contactcolumns))
					{ 
			...
			<?	if(count($indexcontacts) > 0){
					foreach($indexcontacts->items as $contact)
					{
					?><tr id="<?=$contact->id;?>"><?	
						foreach($contact as $key => $value)
						{
							if(in_array($key,$contactcolumns)){ ?><td class="<?=$key;?>"><?=$value?></td><? }
						}
						?>
						<td id="bewerken" style="width:100px;">
						<?	if(in_array('edit',$actions))
							{
								?><a href="<?php echo  $this->url->get("contact/view&id=".$contact->id);?>"><i class="fa fa-search"></i></a><?
							}
							if(in_array('edit',$actions))
							{
								?><a href="<?php echo  $this->url->get('contact/edit&id='.$contact->id);?>"><i class="fa fa-edit"></i></a><?
							}
							if(in_array('delete',$actions))
							{
								?><a onclick="del('<?=$contact->id;?>','<? if(isset($contact->titel)){ echo $contact->titel; } ?>');"><i class="fa fa-trash-o"></i></a><?
							} ?>
					   </td>
					 </tr><?		
					}
	...
	<div class="clearfix">		
		<div class="pull-right">
			...
			<button class="btn btn-primary" onclick="go('<?php echo $this->url->get('contact/new'.$id);?>');">
